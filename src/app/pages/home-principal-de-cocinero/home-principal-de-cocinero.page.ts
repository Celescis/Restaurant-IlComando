...
@Component({
  selector: 'app-home-principal-de-cocinero',
  templateUrl: './home-principal-de-cocinero.page.html',
  styleUrls: ['./home-principal-de-cocinero.page.scss'],
})
export class HomePrincipalDeCocineroPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.mesasSrv.TraerPedidos("aceptado").subscribe((pedidos: any) => {
      this.listadoPedidosAprobados = pedidos.map((pedido: any) => {
        const productosFiltrados = pedido.productos.filter((producto: any) => producto.perfil === "cocinero");
    ...
    this.mesasSrv.TraerPedidos("cocinado").subscribe((pedidos: any) => {
      this.listadoPedidosCocinados = pedidos.filter((pedido: any) => {
        return !pedido.cocineroOk && pedido.bartenderOk && pedido.productos.some((producto: any) => producto.perfil === "cocinero");
      }).map((pedido: any) => {
        const productosFiltrados = pedido.productos.filter((producto: any) => producto.perfil === "cocinero");
        return { ...pedido, productos: productosFiltrados };
      });
  ...
  async EntregarPedido(pedido: any) {
    await this.mesasSrv.UpdatearPedidoCocinero(pedido, true);
